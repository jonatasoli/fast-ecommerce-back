...
async def test_successful_send_to_crm(mocker):
    ...
    mock_requests_post = mocker.patch(
        'requests.post', return_value=AsyncMock(status_code=200),
    )
    ...
    assert (
        user_data.email
        in mock_requests_post.call_args.kwargs['json']['contacts'][0]['emails'][0][
            'email'
        ]
    )
...
async def test_send_to_crm_fails_with_bad_status_code(mocker):
    ...
    mock_requests_post = mocker.patch(
        'requests.post', return_value=AsyncMock(status_code=400),
    )
