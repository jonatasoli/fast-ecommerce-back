...
def test_product_not_found_exception_should_raise():
    """Must raise ProductNotFoundError."""
...
async def test_get_inventory_should_verify_admin_and_return_inventory(mocker, asyncdb):
    """Must verify admin and return inventory."""
    # Arrange
    ...
    # Act
    result = await get_inventory(token, page=page, offset=offset, db=asyncdb, verify_admin=mock_verify)
...
async def test_get_inventory_name_should_return_inventory_by_name(mocker, asyncdb):
    """Must return inventory for product name."""
    # Arrange
    mock_repo = mocker.patch(
        "app.product.repository.get_inventory",
        return_value={"products": [], "total": 0},
    ...
    # Act
    result = await get_inventory_name(path, currency=currency, page=page, offset=offset, db=asyncdb)
...
async def test_inventory_transaction_should_add_transaction(mocker, asyncdb):
    """Must add inventory transaction."""
    # Arrange
    mock_repo = mocker.patch(
    ...
    # Act
    result = await inventory_transaction(product_id, inventory=inventory, token=token, db=asyncdb)
...
async def test_update_product_not_found_should_raise(mocker, asyncdb):
    """Must raise ProductNotFoundError when product not found."""
    # Arrange
...
@pytest.mark.asyncio
async def test_upload_media_gallery_should_upload_video(real_upload_file, mocker, asyncdb):
    """Must upload video to media gallery."""
    # Arrange
    async with asyncdb().begin() as transaction:
        category = CategoryFactory()
        config_fee = CreditCardFeeConfigFactory()
...
async def test_delete_media_gallery_should_delete_media(mocker, asyncdb):
    """Must delete media from gallery."""
    # Arrange
    async with asyncdb().begin() as transaction:
        category = CategoryFactory()
        config_fee = CreditCardFeeConfigFactory()
    ...
    # Act
    await delete_media_gallery(product_db.product_id, media_id=media.media_id, db=asyncdb)
...
async def test_get_media_gallery_should_return_media_list(mocker, asyncdb):
    """Must return list of media for product."""
    # Arrange
    async with asyncdb().begin() as transaction:
        category = CategoryFactory()
        config_fee = CreditCardFeeConfigFactory()
