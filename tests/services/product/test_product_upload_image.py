...
def test_upload_image_should_change_image_path(mocker: MockerFixture, db: Session) -> None:
    ...
    mocker.patch.object(repository, 'get_product_by_id', return_value=product_db)
    image_client_mock = mocker.Mock()
    image_client_mock.optimize_image.return_value = new_image_path

    # Act
    response = upload_image(product_id, db=db, image=fake_file(), image_client=image_client_mock)
