...
def test_create_order(session):
    ...
    # Act
    order = session.scalar(select(Order).where(Order.order_id == new_order.order_id))
...
def test_order_status_steps(session):
    ...
    # Act
    order_status_steps = session.scalar(select(OrderStatusSteps).where(OrderStatusSteps.order_id == new_order_status_steps.order_id))
