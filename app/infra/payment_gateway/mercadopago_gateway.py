...
def create_credit_card_payment(customer_id, amount, card_token, installments, client: SDK=get_payment_client()):
    payment_data = {
...
def cancel_credit_card_reservation(payment_id, client: SDK=get_payment_client()):
    payment_data = {
      "status": "cancelled"
    }
    payment_response = client.payment().update(payment_id, payment_data)
