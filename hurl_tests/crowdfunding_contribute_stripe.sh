...
echo "🚀 Fazendo contribuição via Stripe (Cartão de Crédito) em projeto de crowdfunding..."
echo "User: $USER"
...
echo "3️⃣ Criando PaymentMethod no Stripe..."
STRIPE_TOKEN=$(create_stripe_payment_method "4242424242424242" "12" "2025" "123")

if [ "$STRIPE_TOKEN" = "pm_card_visa" ] && [ -z "$STRIPE_SECRET_KEY" ]; then
    ...
    echo "   ✅ PaymentMethod criado: $STRIPE_TOKEN"
fi
echo ""

echo "4️⃣ Realizando contribuição via Stripe..."
...
echo "✅ Contribuição via Stripe realizada com sucesso!"
echo ""
...
echo "  - Payment Method: $STRIPE_TOKEN"
echo "  - Status: $(echo "$CONTRIBUTION_BODY" | jq -r '.status // "N/A"')"
echo ""
echo "💡 Verifique o status do pagamento no dashboard do Stripe"
